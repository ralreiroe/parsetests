...
class FieldValueSpec extends FlatSpec with Matchers {
  "json" should "parse into FieldValue" in {
    ...
    object Orientation {
      implicit val reads: Reads[Orientation] = Json.reads[Orientation]
    }
    ...
    object FieldValue {
      ...
      val format: Reads[FieldValue] = (
        (__ \ 'id).read[FieldId] and
          (__ \ 'label).read[String] and
          (__ \ 'orientation).read[Orientation]) (FieldValue.apply _)

      implicit val reads: Reads[FieldValue] = (format: Reads[FieldValue]) | FieldValueRaw.reads.flatMap(_.toFieldValue) //(2) which uses Reads[FieldValueRaw] to parse json to FieldValueRaw before converting the latter into a FieldValue
    }
    ...
    case class FieldValueRaw(
      ...
      def convertToFieldValue() = FieldValue(id, label, format match {
        case Some(OrientationFormat("horizontal")) => Horizontal
        case _ => Vertical
      })

      def toFieldValue: Reads[FieldValue] = Reads[FieldValue] { fv => JsSuccess(convertToFieldValue()) }
    ...
    val res = Json.parse(jsonStr).validate[FieldValue]  //(1) handled by a Reads[FieldValue]

    res match {
      case s: JsSuccess[FieldValue] => {
        val place: FieldValue = s.get
        println(place)
      }
      case e: JsError => {
        // error handling flow
