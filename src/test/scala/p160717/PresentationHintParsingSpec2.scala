...
class PresentationHintParsingSpec2 extends FlatSpec with Matchers {
  "collapseGroupUnderLabel,summariseGroupAsGrid" should
    ...
    object PresentationHintParser {
      ...
      lazy val presentationHint: Parser[PresentationHint] = (                                     //leads to better error reporting: expected 'summariseGroupAsGrid' or 'collapseGroupUnderLabel'
        "collapseGroupUnderLabel" ^^ { (loc, unparsed) => PresentationHint(unparsed) } |
        "summariseGroupAsGrid" ^^ { (loc, unparsed) => PresentationHint(unparsed) }
        )
    }
